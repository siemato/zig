...
pub inline fn mulf3(comptime T: type, a: T, b: T) T {
    ...
    const ZSignificand = common.PowerOfTwoSignificandZ(T);
    ...
    if (aExponent -% 1 >= maxExponent - 1 or bExponent -% 1 >= maxExponent - 1) {
        ...
        if (aAbs < minNormalRep) scale += common.normalize(T, &aSignificand);
        if (bAbs < minNormalRep) scale += common.normalize(T, &bSignificand);
    }
